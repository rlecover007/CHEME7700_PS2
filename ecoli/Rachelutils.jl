...
function mass_balances(t, y, data_dictionary)
	...
	data_dictionary=setBounds(data_dictionary, y[94]) #biomass is species #94

	# solve the lp problem -
	(objective_value, flux_array, dual_array, uptake_array, exit_flag) = FluxDriver(data_dictionary)
	@show flux_array[76] #glucose import
	@show flux_array[6], flux_array[11] #acetate production
	@show flux_array[24]
	change =S*flux_array#-flux_array[24]*y
	@show y[73]
	idx_small = find(y.<0)
...
function dFBA()
	...
	inital_conditions[82]=10.0
	inital_conditions[94] = .1 #make biomass non zero
	res=eulerIntegration(inital_conditions, mass_balances, time, data_dictionary)
	@show size(res)
	#ac_c = 7, ac_b, 73
	#biomass 94
	plotdFBAResultsAgainstData(time, data_dictionary, res, [73,82,94])
